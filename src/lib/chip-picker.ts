...
/**
 * ★ PRECOMPILED COMPACT GBNF for the SIDECAR path. llama-server's own
 *   json_schema→grammar conversion allows pretty-printing, and the model
 *   takes it: warm chip answers measured 111 tokens vs the in-process
 *   compact grammar's ~70 — ~35% of generation spent on whitespace. Sending
 *   this grammar directly restores the compact wire.
 *
 *   Generated by node-llama-cpp's own generator (the SAME one the host uses):
 *     getGbnfGrammarForGbnfJsonSchema(CHIP_SCHEMA_RICH, {allowNewLines:false})
 *   with ONE edit: the generator ends root with `"\n\n\n\n" [\n]*` — a stop
 *   marker the host pairs with a stopGenerationTriggers, which llama-server
 *   does not have, so verbatim it would FORCE the model to pad newlines to
 *   the token cap. The tail is stripped; grammar completion is the stop.
 *   REGENERATE whenever CHIP_SCHEMA_RICH changes (see the probe header in
 *   scripts/probe-sidecar-e2e.cjs).
 */
export const CHIP_RICH_GBNF = `root ::= "{" whitespace-no-new-lines-rule "\\"p\\"" ":" [ ]? rule1 whitespace-no-new-lines-rule "}"
...
string-0-72-rule ::= "\\"" ( string-char-rule ){0,72} "\\""
string-0-96-rule ::= "\\"" ( string-char-rule ){0,96} "\\""
whitespace-no-new-lines-rule ::= [ ]?
rule0 ::= "[" whitespace-no-new-lines-rule integer-number-rule comma-whitespace-no-new-lines-rule string-0-72-rule comma-whitespace-no-new-lines-rule string-0-96-rule whitespace-no-new-lines-rule "]"
rule1 ::= "[" whitespace-no-new-lines-rule ( rule0 ( comma-whitespace-no-new-lines-rule rule0 ){0,3} )? whitespace-no-new-lines-rule "]"`;
...
export interface ChipRequest {
  ...
  /** Precompiled compact grammar for the sidecar path (rich only). */
  gbnf?: string;
  maxTokens: number;
}
...
export async function runChipPick(
  ...
  const result = await opts.run<unknown>({
    ...
    // the ★★ on CHIP_SCHEMA_RICH. The gbnf carries the same compact grammar
    // to the sidecar, whose own schema conversion would allow pretty-print.
    ...(opts.rich ? { jsonStyle: "compact" as const, gbnf: request.gbnf } : {}),
    ...(opts.onPartialPicks
      ? {
